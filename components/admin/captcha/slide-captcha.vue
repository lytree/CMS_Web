...
// 开始请求生成图片时调用
function startRequestGenerate() {
  reset()
  state.showGenerateLoadding = true
}
// 结束请求生成图片时调用
function endRequestGenerate(src: string, sliderSrc: string) {
  state.showGenerateLoadding = false
  state.src = src
  state.sliderSrc = sliderSrc
}
// 开始请求校验时调用
function startRequestVerify() {
  state.showVerifyLoadding = true
}
// 结束请求校验时调用
function endRequestVerify(isPassing: boolean) {
...
// 拖拽中
function handleDragMoving(e: any) {
  e?.preventDefault()
  if (state.isMoving && !state.isPassing && state.src && state.sliderSrc && !state.isFinish) {
    const _x = (e.touches ? e.touches[0].pageX : e.clientX) - state.x
    if (_x > 0 && _x <= state.width - props.barHeight) {
      const _y = (e.touches ? e.touches[0].pageY : e.clientY) - state.y
...
// 拖拽结束
function handleDragFinish(e: any) {
  e?.preventDefault()
  if (state.isMoving && !state.isPassing && state.src && state.sliderSrc && !state.isFinish) {
    state.isMoving = false
    state.isFinish = true
...
<template>
  <div class="captcha">
    <div class="captcha__main" :style="imgWrapperStyle">
      <img v-if="state.src" ref="backgroundRef" :src="state.src" class="captcha_background" alt="background">
      <img
      ...
      >
      <div v-if="state.showVerifyTip" class="captcha_message">
        <div class="captcha_message__icon">
          ...
          <svg v-else width="28" height="28" viewBox="0 0 28 28" xmlns="http://www.w3.org/2000/svg">
            <g stroke="#fff" stroke-width="1.5" fill="none" fill-rule="evenodd">
              ...
              <path stroke-linecap="round" stroke-linejoin="round" d="M8.75 8.75l10.5 10.5M19.25 8.75l-10.5 10.5" />
            </g>
          </svg>
        </div>
        <div class="captcha_message__text">
          {{ state.isPassing ? successTip : failTip }}
        </div>
      </div>
      <div v-if="state.showGenerateLoadding" class="captcha_message loadding">
        <div class="captcha_message__icon captcha_message__icon--loadding" />
        <div class="captcha_message__text">
          加载中...
        </div>
      </div>
      <div v-if="state.showVerifyLoadding" class="captcha_message">
        <div class="captcha_message__icon captcha_message__icon--loadding" />
        <div class="captcha_message__text" />
      </div>
    </div>
    <div ref="dragVerify" class="captcha__bar" :style="dragVerifyStyle">
      <div ref="progressBar" class="captcha_progress_bar" :class="{ goFirst2: state.isOk }" :style="progressBarStyle" />
      <div class="captcha_progress_bar__text" :style="textStyle">
        {{ state.tracks.length > 0 || state.isPassing ? '' : text }}
      </div>
      <div
        ref="handler"
    ...
    <div v-if="showRefresh && !state.isPassing" class="captcha__actions">
