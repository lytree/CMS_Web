<script lang="ts" setup>
import { defineAsyncComponent, reactive, ref } from 'vue'
import { CaptchaApi } from '@/server/api/admin/Captcha'

const emits = defineEmits(['ok'])

const SlideCaptcha = defineAsyncComponent(() => import('./slide-captcha.vue'))

const slideCaptchaRef = ref()
const state = reactive({
  requestId: '',
  failTip: '',
  successTip: '',
})

// 生成滑块验证码
async function onGenerate() {
  slideCaptchaRef.value.startRequestGenerate()
  const res = await new CaptchaApi().captchaGeneratePost({ captchaId: state.requestId }).catch(() => {
    slideCaptchaRef.value.endRequestGenerate(null, null)
  })
  if (res?.success && res.data) {
    state.requestId = res.data.id || ''
    slideCaptchaRef.value.endRequestGenerate(res.data.backgroundImage, res.data.sliderImage)
  }
}

// 验证滑块验证码
async function onFinish(data: any) {
  slideCaptchaRef.value.startRequestVerify()
  const res = await new CaptchaApi().captchaCheckPost(data, { captchaId: state.requestId }).catch((err) => {
    console.log("------------", err);
    state.failTip = '服务异常，请稍后重试'
    slideCaptchaRef.value.endRequestVerify(false)
  })
  if (res?.success && res.data) {
    const success = res.data.result === 0
    state.failTip = res.data.result === 1 ? '验证未通过，拖动滑块将悬浮图像正确合并' : '验证超时, 请重新操作'
    state.successTip = '验证通过'
    slideCaptchaRef.value.endRequestVerify(success)
    if (success) {
      // 验证成功
      emits('ok', { captchaId: state.requestId, track: data })
    }
    else {
      setTimeout(() => {
        onGenerate()
      }, 1000)
    }
  }
}

// 刷新滑块验证码
function refresh() {
  slideCaptchaRef.value?.handleRefresh()
}

defineExpose({
  refresh,
})
</script>

<template>
  <SlideCaptcha ref="slideCaptchaRef" :fail-tip="state.failTip" :success-tip="state.successTip" width="100%" height="auto"
    v-bind="$attrs" @refresh="onGenerate" @finish="onFinish" />
</template>

<style scoped></style>
