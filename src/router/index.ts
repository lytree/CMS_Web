...
// 读取 `@/stores/themeConfig.ts` 是否开启后端控制路由配置
const storesThemeConfig = useThemeConfig(usePinia())
  ...
  arr.forEach((v: any) => {
    if (v.path === '/') {
      ...
      if (newArr[0].meta?.isKeepAlive && v.meta?.isKeepAlive) {
        ...
        const stores = useKeepALiveNames(usePinia())
...
useRouter().beforeEach(async (to, from, next) => {
  ...
  if (to.meta.title)
    NProgress.start()
  const storesUseUserInfo = useUserInfo(usePinia())
  ...
  if (to.path === '/login' && !token) {
    ...
    if (!token) {
      ...
      const storesRoute = useRoute(usePinia())
