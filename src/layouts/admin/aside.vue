...
// 关闭移动端蒙版
function closeLayoutAsideMobileMode() {
...
// 设置菜单导航是否固定（移动端）
function initMenuFixed(clientWidth: number) {
  state.clientWidth = clientWidth
}
// 鼠标移入、移出
function onAsideEnterLeave(bool: boolean) {
  ...
  if (themeConfig.value.isColumnsMenuHoverPreload)
...
<template>
  <div v-show="!isTagsViewCurrenFull" class="h-full">
    <el-aside class="layout-aside" :class="setCollapseStyle">
      <Logo v-if="setShowLogo"/>
      <el-scrollbar ref="layoutAsideScrollbarRef" class="flex-auto overflow-hidden" @mouseenter="onAsideEnterLeave(true)" @mouseleave="onAsideEnterLeave(false)">
