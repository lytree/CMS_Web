...
function onEdit(row: UserGetPageOutput) {
  state.userFormTitle = '编辑用户'
  userFormRef.value.open(row)
}

function onDelete(row: UserGetPageOutput) {
  proxy.$modal
...
function onResetPwd(row: UserGetPageOutput) {
  proxy.$modal
...
function onSetManager(row: UserGetPageOutput) {
...
function onSetEnable(row: UserGetPageOutput & { loading: boolean }) {
...
function onOneClickLogin(row: UserGetPageOutput) {
  proxy.$modal
    ...
    <Pane size="80">
      <div class="my-flex-column h100 w100">
        <el-card class="mt8" shadow="never" :body-style="{ paddingBottom: '0' }">
          <el-form style="max-width: 640px" @submit.stop.prevent>
            <el-row :gutter="35">
              <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
                <el-form-item>
                  <MySelectInput v-model="state.pageInput.dynamicFilter" :filters="state.filters" @search="onQuery" />
                </el-form-item>
              </el-col>
              <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
                <el-form-item>
                  ...
                  <el-button v-auth="'api:admin:user:add'" type="primary" icon="ele-Plus" @click="onAdd">
                    新增
                  </el-button>
                </el-form-item>
              </el-col>
          ...
          <el-table v-loading="state.loading" :data="state.userListData" row-key="id" style="width: 100%">
            ...
            <el-table-column prop="name" label="姓名" width="120" show-overflow-tooltip>
              <template #default="{ row }">
                {{ row.name }} <el-tag v-if="row.isManager" type="success">
                  主管
                </el-tag>
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作" width="140" header-align="center" align="center" fixed="right">
              <template #default="{ row }">
                <el-button v-auth="'api:admin:user:update'" icon="ele-EditPen" size="small" text type="primary" @click="onEdit(row)">
                  编辑
                </el-button>
                <MyDropdownMore
        ...
        <UserForm ref="userFormRef" :title="state.userFormTitle" />
      </div>
    </Pane>
  </MyLayout>
</template>
