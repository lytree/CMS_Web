...
function formatterInterval(row: any, column: any, cellValue: any) {
  let label = ''
  switch (cellValue) {
    case 1:
    case 'SEC':
...
// 查看日志
function onShowLogs(row: TaskListOutput) {
  state.taskLogsTitle = `${row.topic}${row.id}运行日志`
  taskLogsRef.value.open(row)
}

function onRun(row: TaskListOutput) {
  proxy.$modal
...
function onPause(row: TaskListOutput) {
  proxy.$modal
...
function onStart(row: TaskListOutput) {
  proxy.$modal
...
function onDelete(row: TaskListOutput) {
  proxy.$modal
...
<template>
  <div class="my-layout">
      ...
      <el-table v-loading="state.loading" :data="state.taskListData" row-key="id" style="width: 100%">
        ...
        <el-table-column prop="status" label="任务状态" width="80">
          <template #default="{ row }">
            <el-tag v-if="row.status === 0 || row.status === 'Running'" disable-transitions>
              运行中
            </el-tag>
            <el-tag v-if="row.status === 1 || row.status === 'Paused'" type="info" disable-transitions>
              暂停
            </el-tag>
            <el-tag v-if="row.status === 2 || row.status === 'Completed'" type="success" disable-transitions>
              完成
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="180" fixed="right" header-align="center" align="center">
          <template #default="{ row }">
            ...
            <el-button v-auth="'api:admin:task:run'" icon="ele-Promotion" size="small" text type="primary" @click="onRun(row)">
              执行
            </el-button>
            <el-button v-auth="'api:admin:task:delete'" icon="ele-Delete" size="small" text type="danger" @click="onDelete(row)">
              删除
            </el-button>
          </template>
        </el-table-column>
    ...
    <TaskLogs ref="taskLogsRef" :title="state.taskLogsTitle" />
  </div>
</template>
