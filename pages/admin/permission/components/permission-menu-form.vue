...
import type { PermissionListOutput, PermissionUpdateMenuInput, ViewListOutput } from '@/api/admin/data-contracts'
...
function onViewFilterNode(value: string, data: ViewListOutput) {
  if (!value)
    return true
  return data.label?.indexOf(value) !== -1 || data.path?.indexOf(value) !== -1
}

function onViewCurrentChange(data: ViewListOutput) {
  if (data) {
    if (!state.form.label)
