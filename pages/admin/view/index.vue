...
function onEdit(row: ViewListOutput) {
  state.viewFormTitle = '编辑视图'
  viewFormRef.value.open(row)
}

function onDelete(row: ViewListOutput) {
  proxy.$modal
...
<template>
  <div class="my-layout">
    <el-card class="mt8" shadow="never" :body-style="{ paddingBottom: '0' }">
      <el-form :inline="true" @submit.stop.prevent>
        ...
        <el-form-item>
          ...
          <el-button v-auth="'api:admin:view:add'" type="primary" icon="ele-Plus" @click="onAdd">
            新增
          </el-button>
        </el-form-item>
      </el-form>
    ...
    <el-card class="my-fill mt8" shadow="never">
      ...
      >
        ...
        <el-table-column label="操作" width="160" fixed="right" header-align="center" align="center">
          <template #default="{ row }">
            <el-button v-auth="'api:admin:view:update'" icon="ele-EditPen" size="small" text type="primary" @click="onEdit(row)">
              编辑
            </el-button>
            <el-button v-auth="'api:admin:view:delete'" icon="ele-Delete" size="small" text type="danger" @click="onDelete(row)">
              删除
            </el-button>
          </template>
        </el-table-column>
    ...
    <ViewForm ref="viewFormRef" :title="state.viewFormTitle" :view-tree-data="state.viewTreeData" />
  </div>
</template>
